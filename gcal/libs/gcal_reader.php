...
class gcal_reader
{
    ...
    public function __construct($cals)
    {
        $this->cals = $cals;
...
function displayReadableDateInPDT($icalDate) {
    ...
    {
//        echo date( "m/d/Y", $end_date) . " - " . date( "m/d/Y", $end_date);
...
*/        
        usort($complete_events, array('gcal_reader','date_compare') );
...
//            echo $status . "<br>";
    ...
    //        $formatted_event = get_fields($description, $fields);
            ...
            $end_date = isset($event['DTEND'])
                ? $cal['ical']->iCalDateToUnixTimestamp($event['DTEND'])
                : $start_date + 86400;
            ...
            $formatted_event['DTEND']        = isset($event['DTEND']) ? $event['DTEND'] : $event['DTSTART'];
